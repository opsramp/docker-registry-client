...
```go
import (
    "github.com/opsramp/docker-registry-client/registry"
...
```go
tags, err := hub.Tags("opsramp/cedar")
```
...
```go
manifest, err := hub.Manifest("opsramp/cedar", "14")
```

Schema V2

```go
manifest, err := hub.ManifestV2("opsramp/cedar", "14")
```
...
```go
digest, err := hub.ManifestDigest("opsramp/cedar", "14")
```
...
```go
digest, err := hub.ManifestDigest("opsramp/cedar", "14")
err = hub.DeleteManifest("opsramp/cedar", digest)
```
...
reader, err := hub.DownloadBlob("opsramp/cedar", digest)
if reader != nil {
    defer reader.Close()
}
if err != nil {
